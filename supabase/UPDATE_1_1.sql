...
create table if not exists public.mpsq_assets (
 id text primary key, kind text not null check(kind in ('model','jar','sound','npc_skin')),
 category text not null default 'shared' check(category in ('shared','furniture','accessory','sound','npc_skin','npc_model','npc_skin_normal','npc_skin_slim','mod_release')),
 behavior text not null default 'decoration' check(behavior in ('decoration','interactive')),
...
alter table public.mpsq_assets add constraint mpsq_assets_category_check check(category in ('shared','furniture','accessory','sound','npc_skin','npc_model','npc_skin_normal','npc_skin_slim','mod_release'));
...
-- MPSQ_MODELS_ACCESSORIES.sql: asset categories and per-world NPC models.
