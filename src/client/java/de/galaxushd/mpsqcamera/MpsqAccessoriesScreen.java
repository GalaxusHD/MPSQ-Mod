...
public final class MpsqAccessoriesScreen extends Screen {
    ...
    @Override protected void init(){
        MpsqApiClient.get("/me/points").whenComplete((d,e)->client.execute(()->{if(e==null&&d.isJsonObject()&&d.getAsJsonObject().has("points")){points=d.getAsJsonObject().get("points").getAsLong();}else points=-1;}));
    ...
    @Override public void render(DrawContext c,int mx,int my,float delta){
        ...
        JsonArray list=rows();for(int i=0;i<list.size();i++){int yy=TOP+i*ROW-scroll;if(yy+ROW<TOP||yy>bottom)continue;JsonObject r=list.get(i).getAsJsonObject();JsonObject def=r.has("mpsq_accessories")&&r.get("mpsq_accessories").isJsonObject()?r.getAsJsonObject("mpsq_accessories"):r;String name=str(r,"display_name",str(def,"display_name",str(r,"id",str(def,"accessory_key","Unbenannt"))));boolean equipped=r.has("equipped")&&r.get("equipped").getAsBoolean();
            ...
            if(tab==1){int price=r.has("price_points")?r.get("price_points").getAsInt():500;label+=(r.has("owned")&&r.get("owned").getAsBoolean()?"  · Besitzt du":"  · "+price+" Punkte");}
    ...
    private void buy(JsonObject row){if(pending||!row.has("accessory_id")||row.get("accessory_id").isJsonNull())return;if(row.has("owned")&&row.get("owned").getAsBoolean()){status="Dieses Accessoire besitzt du bereits.";return;}pending=true;JsonObject body=new JsonObject();body.addProperty("accessoryId",row.get("accessory_id").getAsString());status="Kauf wird verarbeitet …";MpsqApiClient.post("/me/accessories/buy",body).whenComplete((d,e)->client.execute(()->{pending=false;if(e==null){points=d.isJsonObject()&&d.getAsJsonObject().has("points")?d.getAsJsonObject().get("points").getAsLong():points;status="Accessoire gekauft und freigeschaltet.";ownedLoaded=false;catalogLoaded=false;}else status="Kauf fehlgeschlagen: "+e.getMessage();clearAndInit();}));}
