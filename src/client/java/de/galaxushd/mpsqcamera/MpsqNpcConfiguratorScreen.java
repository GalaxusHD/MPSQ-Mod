...
public final class MpsqNpcConfiguratorScreen extends Screen {
    private static final String[] TABS={"Aussehen","Interaktion","Animation"};
    ...
    private int tab;
    private String displayName,glowColor,animation,dialogue,status="";
    private float scale;
    ...
    @Override public void render(DrawContext c,int mouseX,int mouseY,float delta){
        ...
        else {c.drawTextWithShadow(textRenderer,Text.literal("Laufanimation auswählen"),content,top+49,0xFFE8EAF0);for(int i=0;i<ANIMATION_LABELS.length;i++){int bx=content,by=top+73+i*34;c.fill(bx,by,bx+190,by+27,animation.equals(ANIMATIONS[i])?0xFF9C203C:0xFF292E3A);drawPixelIcon(c,bx+7,by+7,i+3,0xFFFF687A);c.drawTextWithShadow(textRenderer,Text.literal(ANIMATION_LABELS[i]),bx+29,by+8,0xFFFFFFFF);if(animation.equals(ANIMATIONS[i]))c.drawTextWithShadow(textRenderer,Text.literal("✓"),bx+171,by+8,0xFFFFA0AA);}}
        c.drawTextWithShadow(textRenderer,textRenderer.trimToWidth(Text.literal(status),right-left-30),left+15,bottom-19,status.startsWith("Gespeichert")?0xFF77DD99:0xFFFFA0AA);
    }
    ...
    private void drawTab(DrawContext c,int x,int y,int index,boolean selected){c.fill(x,y,x+60,y+31,selected?0xFF9C203C:0xFF242A35);drawPixelIcon(c,x+6,y+7,index,selected?0xFFFF9BA7:0xFFB9C0CD);c.drawTextWithShadow(textRenderer,Text.literal(switch(index){case 0->"Optik";case 1->"Text";default->"Pose";}),x+25,y+11,0xFFFFFFFF);}
    ...
    private void save(){if(pending)return;captureFields();List<String> pages=new ArrayList<>();for(String p:dialogue.split("\\|\\|",-1)){String page=p.trim();if(page.isEmpty()||page.length()>240||pages.size()>=12){status="Dialog: 1–12 Seiten, jeweils max. 240 Zeichen.";return;}pages.add(page);}if(displayName.isBlank()){status="Bitte einen NPC-Namen eingeben.";return;}
        JsonArray pageArray=new JsonArray();pages.forEach(pageArray::add);JsonObject interaction=new JsonObject();interaction.add("pages",pageArray);JsonObject body=new JsonObject();body.addProperty("name",displayName);body.addProperty("scale",scale);body.addProperty("glowColor",glowColor);body.addProperty("animation",animation);body.add("interactionData",interaction);body.addProperty("server",server);body.addProperty("world",world);pending=true;status="Wird gespeichert …";clearAndInit();
    ...
    @Override public boolean mouseClicked(double x,double y,int button){if(button==0){int left=panelLeft(),top=43;if(x>=left+10&&x<=left+74&&y>=top+134&&y<top+250){captureFields();tab=Math.max(0,Math.min(2,(int)(y-(top+139))/37));clearAndInit();return true;}if(tab==0&&x>=left+82&&y>=top+156&&y<top+220){int col=(int)(x-(left+82))/23,row=(int)(y-(top+158))/21;int index=row*6+col;if(col>=0&&col<6&&row>=0&&row<3&&index<COLORS.length){captureFields();glowColor=COLORS[index];return true;}}if(tab==2&&x>=left+82&&x<=left+280&&y>=top+73&&y<top+209){int index=(int)(y-(top+73))/34;if(index>=0&&index<ANIMATIONS.length){animation=ANIMATIONS[index];return true;}}}return super.mouseClicked(x,y,button);}
