...
public final class MpsqNpcConfiguratorScreen extends Screen {
    ...
    @Override public boolean mouseClicked(double x,double y,int button){if(button==0){int left=panelLeft(),top=43;if(x>=left+10&&x<=left+74&&y>=top+134&&y<top+324){captureFields();tab=Math.max(0,Math.min(4,(int)(y-(top+139))/37));scroll=0;clearAndInit();return true;}if(tab==0&&x>=left+82&&x<=left+82+138&&y>=top+158-scroll&&y<top+158-scroll+63){int col=(int)(x-(left+82))/23,row=(int)(y-(top+158-scroll))/21;int index=row*6+col;if(col>=0&&col<6&&row>=0&&row<3&&index<COLORS.length){captureFields();glowColor=COLORS[index];return true;}}if(tab==2&&x>=left+82&&x<=left+280&&y>=top+73-scroll&&y<top+209-scroll){int index=(int)(y-(top+73-scroll))/34;if(index>=0&&index<ANIMATIONS.length){animation=ANIMATIONS[index];return true;}}if(tab==4&&x>=panelLeft()+82&&y>=top+68-scroll&&y<top+94-scroll){int current=Math.max(0,java.util.Arrays.asList(TASKS).indexOf(taskType));taskType=TASKS[(current+1)%TASKS.length];clearAndInit();return true;}}return super.mouseClicked(x,y,button);}
    @Override public boolean mouseScrolled(double x,double y,double horizontal,double vertical){int max=tab==0?Math.max(0,230-(height-104)):tab==1?Math.max(0,130-(height-104)):0;captureFields();scroll=Math.max(0,Math.min(max,scroll-(int)Math.signum(vertical)*24));clearAndInit();return true;}
