...
public final class MpsqNpcConfiguratorScreen extends Screen {
    private static final String[] TABS={"Aussehen","Interaktion","Animation","Drehung"};
    ...
    private void drawTab(DrawContext c,int x,int y,int index,boolean selected){c.fill(x,y,x+60,y+31,selected?0xFF9C203C:0xFF242A35);drawPixelIcon(c,x+6,y+7,index,selected?0xFFFF9BA7:0xFFB9C0CD);c.drawTextWithShadow(textRenderer,Text.literal(switch(index){case 0->"Optik";case 1->"Text";case 2->"Pose";default->"Winkel";}),x+25,y+11,0xFFFFFFFF);}
    ...
    @Override public boolean mouseClicked(double x,double y,int button){if(button==0){int left=panelLeft(),top=43;if(x>=left+10&&x<=left+74&&y>=top+134&&y<top+287){captureFields();tab=Math.max(0,Math.min(3,(int)(y-(top+139))/37));clearAndInit();return true;}if(tab==0&&x>=left+82&&y>=top+156&&y<top+220){int col=(int)(x-(left+82))/23,row=(int)(y-(top+158))/21;int index=row*6+col;if(col>=0&&col<6&&row>=0&&row<3&&index<COLORS.length){captureFields();glowColor=COLORS[index];return true;}}if(tab==2&&x>=left+82&&x<=left+280&&y>=top+73&&y<top+209){int index=(int)(y-(top+73))/34;if(index>=0&&index<ANIMATIONS.length){animation=ANIMATIONS[index];return true;}}}return super.mouseClicked(x,y,button);}
