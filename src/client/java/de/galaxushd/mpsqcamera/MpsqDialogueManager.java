...
public final class MpsqDialogueManager {
    ...
    public static void start(JsonObject data){pages.clear();JsonArray input=data.getAsJsonArray("pages");if(input==null)return;for(JsonElement e:input)if(e.isJsonPrimitive()){String line=e.getAsString().trim();if(!line.isEmpty())pages.add(line);}page=0;visibleChars=0;ticks=0;active=!pages.isEmpty();}
    private static void next(){if(page>=pages.size())return;if(visibleChars<pages.get(page).length()){visibleChars=pages.get(page).length();return;}page++;visibleChars=0;if(page>=pages.size())active=false;}
