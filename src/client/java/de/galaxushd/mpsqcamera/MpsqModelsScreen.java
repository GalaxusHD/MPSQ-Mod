...
public final class MpsqModelsScreen extends Screen {
    ...
    @Override public boolean mouseClicked(double x,double y,int button){if(button==0&&x>=width/2+153&&x<=width/2+162&&y>=TOP&&y<height-BOTTOM&&maxScroll()>0){draggingScrollbar=true;scrollTo(y);return true;}if(button==0&&y>=49&&y<66){tab=x<width/2?0:1;scroll=0;return true;}if(button==0&&tab==1&&x>=width/2-158&&x<=width/2+185&&y>=TOP&&y<height-BOTTOM){int center=width/2,rowY=TOP-scroll;for(String cat:new String[]{"furniture","npc_model","npc_skin_normal","npc_skin_slim"}){JsonArray rows=filter(cat);if(rows.isEmpty())continue;rowY+=21;for(JsonElement e:rows){if(y>=rowY&&y<rowY+ROW){JsonObject item=e.getAsJsonObject();String id=str(item,"id",str(item,"asset_id",""));if(!id.isBlank()){client.keyboard.setClipboard(id);status="ID kopiert: "+id;}return true;}rowY+=ROW;}}}if(button==0&&y>=height-25){close();return true;}return super.mouseClicked(x,y,button);}
