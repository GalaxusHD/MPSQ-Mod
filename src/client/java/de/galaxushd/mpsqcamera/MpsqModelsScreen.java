...
public final class MpsqModelsScreen extends Screen {
    ...
    private int tab,scroll;private String status="Lade Modelle …";private boolean requested,draggingScrollbar;
    ...
    @Override protected void init(){if(requested)return;requested=true;
        MpsqApiClient.get("/models/catalog").whenComplete((d,e)->client.execute(()->{if(e==null&&d.isJsonArray()){assets=d.getAsJsonArray();status="";}else status="Modellkatalog konnte nicht geladen werden.";}));
        ...
        MpsqApiClient.get("/objects"+scope).whenComplete((d,e)->client.execute(()->{if(e==null&&d.isJsonArray())placed=d.getAsJsonArray();else if(status.isBlank())status="Platzierte Möbel konnten nicht geladen werden.";}));
    ...
    @Override public void render(DrawContext c,int mx,int my,float delta){super.render(c,mx,my,delta);int center=width/2;
        ...
        else {for(String cat:new String[]{"furniture","npc_model","npc_skin_normal","npc_skin_slim"}){String label=label(cat);JsonArray rows=filter(cat);if(rows.size()==0)continue;y=heading(c,center,y,label);for(JsonElement el:rows){JsonObject o=el.getAsJsonObject();if(y+ROW>=TOP&&y<height-BOTTOM){c.fill(center-153,y,center+153,y+ROW-2,0x44000000);String name=str(o,"display_name",str(o,"name",str(o,"id",str(o,"filename","Asset"))));c.drawTextWithShadow(textRenderer,textRenderer.trimToWidth(Text.literal(name+"  §7["+str(o,"id",str(o,"asset_id","?"))+"]"),294),center-145,y+6,0xFFFFFFFF);}y+=ROW;}}}
