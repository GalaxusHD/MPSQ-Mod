...
public final class MpsqModelsScreen extends Screen {
    ...
    @Override public void render(DrawContext c,int mx,int my,float delta){super.render(c,mx,my,delta);int center=width/2;
        ...
        else {for(String cat:new String[]{"furniture","npc_model","npc_skin_normal","npc_skin_slim"}){String label=label(cat);JsonArray rows=filter(cat);if(rows.size()==0)continue;y=heading(c,center,y,label);for(JsonElement el:rows){JsonObject o=el.getAsJsonObject();if(y+ROW>=TOP&&y<height-BOTTOM){c.fill(center-153,y,center+153,y+ROW-2,0x44000000);String name=str(o,"display_name",str(o,"name",str(o,"id",str(o,"filename","Asset"))));String assetId=str(o,"id",str(o,"asset_id","?"));c.drawTextWithShadow(textRenderer,textRenderer.trimToWidth(name+"  ["+assetId+"]",294),center-145,y+6,0xFFFFFFFF);}y+=ROW;}}}
