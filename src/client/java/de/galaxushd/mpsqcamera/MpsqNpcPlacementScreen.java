...
public final class MpsqNpcPlacementScreen extends Screen {
    ...
    private void save(boolean remove){if(pending)return;JsonObject b=new JsonObject();b.addProperty("server",server);b.addProperty("world",world);b.addProperty("x",pos.getX());b.addProperty("y",pos.getY());b.addProperty("z",pos.getZ());b.addProperty("positionX",worldX);b.addProperty("positionY",worldY);b.addProperty("positionZ",worldZ);b.addProperty("yaw",yaw);b.addProperty("pitch",pitch);b.addProperty("facePlayer",facePlayer);b.addProperty("assetId",asset.getText().trim());b.addProperty("remove",remove);pending=true;status="Wird gespeichert…";MpsqApiClient.post("/npcs",b).whenComplete((d,e)->client.execute(()->{pending=false;status=e==null?"Gespeichert.":"NPC konnte nicht gespeichert werden: "+e.getMessage();if(e==null){MpsqAccessoryRenderer.refresh();if(!remove&&d.isJsonArray()&&!d.getAsJsonArray().isEmpty()){client.setScreen(new MpsqNpcConfiguratorScreen(this,d.getAsJsonArray().get(0).getAsJsonObject()));}}}));}
