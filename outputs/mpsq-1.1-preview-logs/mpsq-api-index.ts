...
async function addRankLog(actorId: string | null, targetId: string, before: any, after: any, action: string, requestId: string | null = null) {
  // Only permanent changes made for another player belong in the visible log.
  if (actorId && actorId === targetId) return;
  if (action.startsWith("TEMPORARY") || action === "EVENT_001" || action === "AUTO_VIP") return;
...
serve(async req => {
  ...
  try {
    ...
    if (path === "/team/ranks" && req.method === "GET") {
      const ranks = [
        ["vip", "VIP", "Unterstützt das Team und erhält Zugriff auf freigegebene Informationen.", ["Team-Chat lesen"]],
        ["spieler", "Spieler", "Standardrang ohne Verwaltungsrechte.", []],
        ["streamer", "Streamer", "Darf freigegebene Kameras und Bildschirme verwenden.", ["Kameras ansehen"]],
        ["soldat", "Soldat", "Unterstützt bei Aufgaben und moderierten Abläufen.", ["Kameras ansehen", "Aufgaben ansehen"]],
        ["arbeiter", "Arbeiter", "Bearbeitet zugewiesene Teamaufgaben.", ["Kameras ansehen", "Aufgaben bearbeiten"]],
        ["offizier", "Offizier", "Darf Teamaufgaben und Rangänderungen im erlaubten Bereich verwalten.", ["Kameras ansehen", "Aufgaben bearbeiten", "Rangänderungen beantragen"]],
        ["frontman", "Frontman", "Leitet Teamabläufe und darf Rangänderungen im erlaubten Bereich verwalten.", ["Kameras ansehen", "Aufgaben bearbeiten", "Rangänderungen beantragen", "Logs ansehen"]],
        ["sr_offizier", "Sr Offizier", "Verantwortlicher Administrationsrang mit vollständiger Rangverwaltung.", ["Alle Teamfunktionen", "Ränge verwalten", "Logs ansehen"]]
      ];
      return out(ranks.map(([id, name, description, permissions]) => ({ id, name, description, permissions })));
    }
    ...
    if (path === "/team/rank-logs" && req.method === "GET") {
      const self = await teamProfile(clientId);
      if (level(permissionRank(self)) < level("offizier")) return out({ error: "Forbidden" }, 403);
      const rows = await (await rest("/mpsq_team_rank_log?select=*&order=created_at.desc&limit=100")).json();
      const ids = [...new Set(rows.flatMap((row: any) => [row.actor_id, row.target_id]).filter(Boolean))];
      const users = ids.length ? await (await rest(`/mpsq_clients?id=in.(${ids.join(",")})&select=id,display_name`)).json() : [];
      const names = new Map(users.map((row: any) => [row.id, row.display_name]));
      return out(rows.map((row: any) => ({ ...row, actor_name: names.get(row.actor_id) ?? "System", target_name: names.get(row.target_id) ?? "Unbekannt" })));
